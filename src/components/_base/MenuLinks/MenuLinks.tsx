...
const getMenuItemRef = (
  href: string,
  refs: Record<string, React.RefObject<HTMLAnchorElement>>,
): React.RefObject<HTMLAnchorElement> => {
  switch (href) {
    case Links.ADMIN.href:
      return refs.adminRef
    case Links.MISSION.href:
      return refs.missionRef
    case Links.EVENTS.href:
      return refs.eventsRef
    case Links.JOURNAL.href:
      return refs.journalRef
            ...
            >
              ...
              <div className='grid grid-cols-2 gap-3'>
                {filteredItems?.map((item, i) => {
                  const itemRef = getMenuItemRef(item?.href, refs)

                  return (
                    <div key={item.href} ref={itemRef}>
                      <motion.div
                        custom={i}
...
export interface MenuLinksHandle {
  openMenu: () => void
  closeMenu: () => void
  toggleMenu: () => void
}
...
const MenuLinksComponent: FCC & React.ForwardRefExoticComponent<any>
  = React.forwardRef<MenuLinksHandle>((_, ref) => {
    ...
    useImperativeHandle(
      ref,
      () => ({
        openMenu: () => setIsModalOpen(true),
        closeMenu: () => setIsModalOpen(false),
        toggleMenu: () => setIsModalOpen((prev) => !prev),
      }),
      [],
    )
            ...
            >
              ...
              >
                ...
                <motion.div
                  ...
                  initial={false}
                  animate={
                    isActive(item.href) ? { opacity: 1 } : { opacity: 0 }
                  }
                />
              </Link>
            </motion.div>
...
MenuLinksComponent.displayName = 'MenuLinks'

const MenuLinks = MenuLinksComponent as React.ForwardRefExoticComponent<
  React.PropsWithoutRef<{}> & React.RefAttributes<MenuLinksHandle>
> & { displayName: string }
